...
function [SE, Vmm, Viv, Wiv, Wmicro,dropParameter,Out_MIndex]=GetCovariance(Theta, SModel,students,option)
...
Jiv=SModel.IV'*[-SModel.XX dDeltadThetanorm]/n;
...
for year=reshape(SModel.Years,1,length(SModel.Years))
    ...
    Simga_i=zeros(3,3);
    Simga_i(1,1)=nanmean(dev_1.*dev_1); d1=(Simga_i(1,1)<10^-6 | n1<10);
    Simga_i(2,2)=nanmean(dev_2.*dev_2); d2=(Simga_i(2,2)<10^-6 | n2<10);   
    Simga_i(3,3)=nanmean(dev_3.*dev_3); d3=(Simga_i(3,3)<10^-6 | n3<10);
...
[Dg, M, G,Jm, flag, dMdEps] = gradient_MicroM_NFP(exp(SModel.delta),Theta2Star, dS_dTheta,dS_ddelta);
